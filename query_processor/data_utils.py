...
def memory_data(data, word_idx, sentence_size, memory_size):
    ...
    for d in data:
        ...
        for i in xrange(len(ss) / memory_size + 1):
            ...
            y = d.get("y")[i*memory_size : (i+1)*memory_size]
            lm = max(0, memory_size - len(y))
            for _ in xrange(lm):
                y.append(0.0)
            A.append(y)
    return np.array(S), np.array(Q), np.array(A)
...
def vectorizeyoyo_data(data, word_idx, sentence_size, memory_size):
    """
    ...
    for story, query, answer in data:
        ...
        # take only the most recent sentences that fit in memory
        for i in xrange(len(ss) / memory_size):
            stmp = ss[i * memory_size : (i+1) * memory_size]
            lm = max(0, memory_size - len(stmp))
            for _ in xrange(lm):
                stmp.append([0] * sentence_size)
...
def bAbI_transform_data(data, word_idx, sentence_size, memory_size):
    S = []
    Q = []
    A = []
    for story, query, answer in data:
        ...
        y = np.zeros(memory_size, dtype=np.float32) # 0 is reserved for nil word
